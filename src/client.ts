...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['FINCH_LOG'] or 'warn' if it isn't set.
   */
...
export class Finch {
   ...
   */
  constructor({
    baseURL = readEnv('FINCH_BASE_URL'),
    accessToken = null,
    clientID = readEnv('FINCH_CLIENT_ID') ?? null,
    clientSecret = readEnv('FINCH_CLIENT_SECRET') ?? null,
    ...
    this.timeout = options.timeout ?? Finch.DEFAULT_TIMEOUT /* 1 minute */;
  ...
  protected validateHeaders({ values, nulls }: NullableHeaders) {
    ...
    if (this.clientID && this.clientSecret && values.get('authorization')) {
      return;
    }
    if (nulls.has('authorization')) {
      return;
