  ...
  for await (const file of walk(path.resolve(__dirname, '..', 'dist'))) {
    ...
    let transformed = mapModulePaths(code, (importPath) => {
      if (file.startsWith(distSrcDir)) {
        if (importPath.startsWith('@tryfinch/finch-api/')) {
          // convert self-references in dist/src to relative paths
          let relativePath = path.relative(
            path.dirname(file),
            path.join(distSrcDir, importPath.substring('@tryfinch/finch-api/'.length)),
          );
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from '@tryfinch/finch-api/_shims/index'`,
