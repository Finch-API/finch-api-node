...
export const parseClientAuthHeaders = (req: IncomingMessage, required?: boolean): Partial<ClientOptions> => { if (req.headers.authorization) {
  ...
  switch (scheme) {
    ...
    case 'Basic':
     ...
     return { clientID: rawValue.slice(0, rawValue.search(':')), clientSecret: rawValue.slice(rawValue.search(':') + 1) }
    default:
      throw new Error('Unsupported authorization scheme. Expected the "Authorization" header to be a supported scheme (Bearer, Basic).');
  }
} else if (required) {
  throw new Error('Missing required Authorization header; see WWW-Authenticate header for details.');
}

const clientID = Array.isArray(req.headers['x-finch-client-id']) ? req.headers['x-finch-client-id'][0] : req.headers['x-finch-client-id']
const clientSecret = Array.isArray(req.headers['x-finch-client-secret']) ? req.headers['x-finch-client-secret'][0] : req.headers['x-finch-client-secret']
